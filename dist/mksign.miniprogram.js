(function(){(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, global.util) :
  ...
  var DataView$1 = getNative$1(root$1, 'DataView'),
      Map$1 = getNative$1(root$1, 'Map'),
      Promise$1 = getNative$1(root$1, 'Promise'),
  ...
  var dataViewCtorString$1 = toSource$1(DataView$1),
      ...
      promiseCtorString$1 = toSource$1(Promise$1),
  ...
  if ((DataView$1 && getTag$1(new DataView$1(new ArrayBuffer(1))) != dataViewTag$2) ||
      (Map$1 && getTag$1(new Map$1) != mapTag$3) ||
      (Promise$1 && getTag$1(Promise$1.resolve()) != promiseTag$1) ||
      (Set$1 && getTag$1(new Set$1) != setTag$3) ||
  ...
  function sort(){var params=0<arguments.length&&arguments[0]!==void 0?arguments[0]:{};if(!isObject_1(params)||isEmpty_1(params))throw new Error("params is not Object or no value in sort function!");return lodash_topairs(params).sort(function(pre,after){return "".concat(pre)>"".concat(after)?1:-1})}

  function filterNull(){var params=0<arguments.length&&void 0!==arguments[0]?arguments[0]:{};if(!isObject_1(params)||isEmpty_1(params))throw new Error("params is not Object or no value in filterNull function!");var tmp={};return Object.keys(params).forEach(function(key){var val=params[key];(0===val||val)&&(tmp[key]=val);}),tmp}
  ...
  function getSignStr(){var params=0<arguments.length&&void 0!==arguments[0]?arguments[0]:[[]],noKeys=[],strArray=[];return params.forEach(function(_ref){var _ref2=slicedToArray(_ref,2),key=_ref2[0],val=_ref2[1];if(0!==val&&!val)throw new Error("param.".concat(key," have not value! pls check!!"));0===key||key?strArray.push(isObject_1(val)||isArray_1(val)?"".concat(key,"=").concat(JSON.stringify(val)):"".concat(key,"=").concat(val)):noKeys.push(val);}),"".concat(strArray.join("&")).concat(noKeys.join(""))}

  function defaultGetSignData(){var params=0<arguments.length&&void 0!==arguments[0]?arguments[0]:{},lastParams=1<arguments.length&&void 0!==arguments[1]?arguments[1]:[],filterData=sort(filterNull(params));if(!isArray_1(lastParams))throw new Error("The second argument is not Array or no value in defaultGetSignData function!");return lastParams.forEach(function(param){if(isObject_1(param)){if(!isObject_1(param)||isEmpty_1(param))throw new Error("The second argument's is not object or no value in defaultGetSignData function!");lodash_topairs(param).forEach(function(_ref){var _ref2=slicedToArray(_ref,2),key=_ref2[0],val=_ref2[1];filterData.push([key,val]);});}else{if(0!==param&&!param)throw new Error("The second argument's no value in defaultGetSignData function!");filterData.push(["",param]);}}),getSignStr(filterData)}
...
}).apply(global)//# sourceMappingURL=mksign.miniprogram.js.map
